...
public class DLList<T extends Comparable<T>> {
    ...
    private void insertionSortHelper(T item) {
        ...
        while (ptr != sentinel && ptr.prev != sentinel && ptr.prev.item.compareTo(item) > 0) {
            T prevItem = ptr.prev.item;
    ...
    public DLList<T> quicksort() {
        ...
        DLList<T> left = smallElements.quicksort();
        DLList<T> right = largeElements.quicksort();
