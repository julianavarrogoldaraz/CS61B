...
public class SLList {
    ...
    public void add(int index, int x) {
        ...
        while (counter < index && pointer.next != sentinel) {
            pointer = pointer.next;
            counter += 1;
        }

        if (index > size) {
            pointer.next = new IntListNode(x, sentinel);
        } else {
    ...
    public void reverse() {
        IntListNode reversed = reverseHelper(sentinel.next);
